...
class AVGfomer(nn.Module):
    def __init__(self,fpn_dim,stride=2,num_heads=3):
        super().__init__()
        self.num_heads=num_heads
        self.avg_pool=nn.AvgPool1d(3,1,1)
        self.conv=nn.Sequential(nn.Conv1d(fpn_dim,int(fpn_dim/4),3,1,1),
                                nn.ReLU(),
                                nn.Conv1d(int(fpn_dim/4),int(fpn_dim/4),3,1,1),)
        self.avg_pool_2=nn.AvgPool1d(3,1,1)
        self.mask_conv=MaskedConv1D(fpn_dim,fpn_dim,3,1,1)
        self.avg_norm=LayerNorm(int(fpn_dim/4))

        self.avg_conv=nn.Conv1d(int(fpn_dim/4),1,3,2,1)

        self.interpolation_parameter=nn.Parameter(torch.randn(1, fpn_dim,1))

        self.query = nn.ModuleList(
            [MaskedConv1D(int(fpn_dim / 4), int(fpn_dim / 4), 3, 1, 1) for _ in range(num_heads)])
        self.key = nn.ModuleList([MaskedConv1D(int(fpn_dim / 4), int(fpn_dim / 4), 3, 1, 1) for _ in range(num_heads)])
        self.value = nn.ModuleList(
            [MaskedConv1D(int(fpn_dim / 4), int(fpn_dim / 4), 3, 1, 1) for _ in range(num_heads)])


        self.attn_mlp=nn.Sequential(nn.Conv1d(int(fpn_dim/4),int(fpn_dim/4),3,1,1),
                                nn.ReLU(),
                                nn.Conv1d(int(fpn_dim/4),fpn_dim,3,1,1),)

    def forward(self, x, mask):
        B,C,T=x.size()
        interpolation_token = repeat(self.interpolation_parameter, '1 C 1 -> B C 1', B=B, C=C)
        avg_pool = torch.cat((x,interpolation_token), dim=2)
        avg_pool=self.avg_pool(avg_pool)
        avg_pool=self.conv(avg_pool)
        avg_pool=self.avg_pool_2(avg_pool)
        avg_pool=self.avg_norm(avg_pool)



        out_tensor=torch.zeros(x.size()).to(avg_pool.device)
        interpolation_frame=avg_pool[:,:,-1]
        avg_pool=avg_pool[:,:,:-1]
        avg_pool_value=self.avg_conv(avg_pool)
        avg_pool_value=torch.sum(avg_pool_value,dim=2)

        for b_idx in range(avg_pool.size(0)):
            interpolation_size=interpolation_frame[b_idx,:]
            interpolation_size=F.relu(interpolation_size)
            interpolation_size=torch.mean(interpolation_size,dim=0)*avg_pool_value[b_idx]
            interpolation_size=F.relu(interpolation_size)
            non_zero_value = torch.ones(interpolation_size.size()).to(x.device)
            interpolation_size = int(interpolation_size) + non_zero_value
            if T<interpolation_size:
                interpolation_size = int(T/interpolation_size)

                interpolation_mask = F.interpolate(
                    mask.to(x.dtype),
                    size=torch.div(T*int(interpolation_size), 1, rounding_mode='trunc'),
                    mode='nearest'
                ).detach()

                interpolation_tensor= torch.nn.functional.interpolate(avg_pool[b_idx].unsqueeze(0), size=T*int(interpolation_size), mode='nearest')
                attended_values = []
                for head_idx in range(self.num_heads):
                    # Multi-Head Attention
                    query, _ = self.query[head_idx](interpolation_tensor, interpolation_mask[b_idx])
                    key, _ = self.key[head_idx](interpolation_tensor, interpolation_mask[b_idx])
                    value, _ = self.value[head_idx](interpolation_tensor, interpolation_mask[b_idx])
                    attn_score = torch.softmax((query @ key.transpose(-2, -1)) / (value.size(2) ** (1 / 2)), dim=-1)
                    attn = attn_score @ value
                    attn = self.attn_mlp(attn)
                    attended_values.append(attn)

                attended_values = torch.stack(attended_values)
                attn = torch.mean(attended_values, dim=0)
                avg_down_sample=nn.AvgPool1d(3,int(interpolation_size),1)
                attn=avg_down_sample(attn)
                out_tensor[b_idx] = attn
            else:
                down_sample_value=nn.MaxPool1d(3,int(T/interpolation_size),1)
                down_sample_tensor =down_sample_value(avg_pool[b_idx].unsqueeze(0))
                interpolation_mask = F.interpolate(
                    mask.to(x.dtype),
                    size=torch.div(down_sample_tensor.size(2), 1, rounding_mode='trunc'),
                    mode='nearest'
                ).detach()
                ...
                for head_idx in range(self.num_heads):
                    # Multi-Head Attention
                    query, _ = self.query[head_idx](down_sample_tensor , interpolation_mask[b_idx])
                    key, _ = self.key[head_idx](down_sample_tensor , interpolation_mask[b_idx])
                    value, _ = self.value[head_idx](down_sample_tensor , interpolation_mask[b_idx])
                    attn_score = torch.softmax((query @ key.transpose(-2, -1)) / (value.size(2) ** (1 / 2)), dim=-1)
                    attn = attn_score @ value
                    attn = self.attn_mlp(attn)
                    attended_values.append(attn)
                ...
                attended_values = torch.stack(attended_values)
                attn = torch.mean(attended_values, dim=0)
                attn = torch.nn.functional.interpolate(attn, size=int(T), mode='nearest')
                out_tensor[b_idx] =attn
        out= x+out_tensor


        out,mask=self.mask_conv(out,mask)

        return out, mask

class ShortLongBlock(nn.Module):
    def __init__(self,fpn_dim,stride):
        super().__init__()
        self.stride=stride
        self.avg_former=nn.ModuleList()
        self.down_sample=nn.AvgPool1d(3,stride,1)
        self.layer_norm=LayerNorm(fpn_dim)
        for i in range(1):
            self.avg_former.append(AVGfomer(fpn_dim,stride))
        #self.avg_former.append(AVGfomer(fpn_dim, stride))

    def forward(self,x,mask):
        B, C, T = x.size()
        fc_pool=self.down_sample(x)
        out_mask = F.interpolate(
            mask.to(x.dtype),
            size=torch.div(T, self.stride, rounding_mode='trunc'),
            mode='nearest'
        ...
        out=fc_pool
        #out, _ = self.avg_former[0](fc_pool, out_mask)
        for idx in range(len(self.avg_former)):
            out,_=self.avg_former[idx](out,out_mask)
            out=out+fc_pool
